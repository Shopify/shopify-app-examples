...
export function CodeIndex() {
  ...
  const rowMarkup = QRCodes.map(
    ({id, title, product, discount, scans, conversions}, index) => (
        <IndexTable.Row id={id} key={id} position={index}>
          ...
          <IndexTable.Cell>{discount?.codeDiscount.codes.edges[0].node.code}</IndexTable.Cell>
          <IndexTable.Cell>{dayjs(Date.now()).format('MMMM D, YYYY')}</IndexTable.Cell>
